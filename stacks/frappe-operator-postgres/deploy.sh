...
################################################################################
# This listing provisions PostgreSQL, not MariaDB, so the StackGres operator has
# to exist before the Frappe Operator can create SGCluster resources. Installed
# here rather than as a chart dependency because helm packages every declared
# dependency into charts/ whether or not its condition is met -- the MariaDB
# listing would carry StackGres it never uses.
#
# Kept out of the --atomic call below on purpose: a rollback of the Frappe
# Operator release must not take a PostgreSQL operator, and any clusters under
# it, down with it.
